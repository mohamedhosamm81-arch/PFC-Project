...
const displayName = key => DESTINATIONS.some(item => item.key === key) ? destination(key).label : String(key || '—');
...
function sessions() {
  ...
  if (!current && user.role !== 'Manager' && user.role !== 'Receptionist') {
    return `<div class="session-start-page"><div class="session-start-brand"><div class="h-14 w-14 rounded-2xl bg-white flex items-center justify-center overflow-hidden p-1 shadow-soft"><img src="eha-logo-mark.png" alt="EHA" class="h-full w-full object-contain" /></div><div><div class="font-bold text-xl">PFC Operations</div><div class="text-xs text-slate-400 uppercase tracking-widest">${esc(user.org || ORG_DEFAULT)}</div></div></div><div class="session-start-card"><div class="h-20 w-20 rounded-3xl bg-teal/10 text-teal flex items-center justify-center mx-auto">${icon('play', 'h-10 w-10')}</div><p class="text-teal font-semibold text-sm mt-7">${esc(displayName(user.dept))} station</p><h1 class="text-4xl font-bold mt-2">Start your session</h1><p class="text-slate-500 text-lg mt-4 max-w-md mx-auto">Welcome, ${esc(user.name)}. Start your shift to become available and receive patients. Your start time and lateness will be recorded automatically.</p><div class="grid grid-cols-2 gap-3 max-w-sm mx-auto mt-7 text-left"><div class="rounded-2xl bg-mist p-4"><div class="text-xs text-slate-400">Shift starts</div><b class="block mt-1">${esc(user.shift || '08:00')}</b></div><div class="rounded-2xl bg-mist p-4"><div class="text-xs text-slate-400">Station status</div><b class="block mt-1 text-slate-500">Unavailable</b></div></div><button data-action="session-toggle" class="mt-8 rounded-2xl bg-teal text-white px-10 py-4 text-lg font-bold hover:bg-teal-700 transition shadow-soft">Start working now ${icon('arrow-right')}</button><p class="text-xs text-slate-400 mt-5">You can pause or end the session at any time.</p></div></div>`;
  }
...
function activityRows() {
  const rows = state.actions.map(action => ({ time: action.time, user: action.user, dept: displayName(action.dept), type: action.type, code: action.code || '—', destination: displayName(action.to || action.dept) }));
  state.sessions.forEach(session => { rows.push({ time: session.start, user: session.user, dept: displayName(session.dept), type: 'Start session', code: '—', destination: displayName(session.dept) }); if (session.end) rows.push({ time: session.end, user: session.user, dept: displayName(session.dept), type: 'End session', code: '—', destination: displayName(session.dept) }); });
