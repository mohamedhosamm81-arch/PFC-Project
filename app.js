...
function queues() {
  const user = currentUser();
  const visible = user.role === 'Manager' ? organizationDestinations(user.org).map(item => item.key) : [user.dept];
  const userOrg = user.org || ORG_DEFAULT;
...
function displays() {
  ...
  if (displayMode === 'door') {
    const doorCards = organizationDestinations(currentUser().org).filter(item => item.key !== 'Reception').map(displayCard).join('');
...
function admin() {
  ...
  const destinationOptions = allDestinations().map(item => `<option value="${esc(item.key)}">${esc(item.label)} — ${esc(item.ar || '')}</option>`).join('');
