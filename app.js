...
const DESTINATIONS = [
  { key: 'Reception', label: 'Reception', ar: 'الاستقبال', icon: 'clipboard-list', tone: 'violet' },
  { key: 'CMO', label: 'CMO', ar: 'العيادات الخارجية', icon: 'stethoscope', tone: 'teal' },
...
function createSeed() {
  const d = today();
  const users = [
    { name: 'System Manager', id: 'ADM-001', dept: 'Administration', org: ORG_DEFAULT, email: 'manager@eha.gov.eg', username: 'manager', password: 'manager123', role: 'Manager', shift: '08:00', availability: 'available' },
  ...
  return {
    ...
    nextCode: 314,
    patients,
    sessions: users.filter(u => !['Manager', 'Receptionist'].includes(u.role)).map((u, i) => ({ userId: u.id, user: u.name, dept: u.dept, org: u.org, start: stamp(['08:01', '08:03', '08:06', '08:14', '08:07', '08:16'][i] || '08:00', d), end: null, shift: u.shift, lateMinutes: Math.max(0, minutesFromTime(['08:01', '08:03', '08:06', '08:14', '08:07', '08:16'][i] || '08:00') - minutesFromTime(u.shift)) })),
    actions: [
...
const employees = () => state.users.filter(user => user.role !== 'Manager' && user.dept !== 'Administration');
...
function pageTitle(target) {
  return ({ overview: 'Operations overview', kiosk: 'Reception ticket desk', queues: 'Queues & patient routing', displays: 'Live display boards', sessions: 'My session', reports: 'Reports & attendance', activity: 'Action sheet', admin: 'Admin dashboard' }[target] || 'Operations overview');
}
function nav() {
  const all = [
...
function sessionSummary() {
  ...
  return `<div class="bg-ink rounded-2xl shadow-soft p-5 text-white"><div class="flex items-center justify-between"><div><h2 class="font-bold">Your station session</h2><p class="text-slate-300 text-sm mt-1">${esc(user.dept)} · ${esc(user.org || ORG_DEFAULT)}</p></div>${icon('scan-line', 'h-5 w-5 text-teal-300')}</div><div class="mt-7 grid grid-cols-2 gap-4"><div><div class="text-xs text-slate-400">Started</div><div class="text-xl font-bold mt-1">${session ? fmtTime(session.start) : 'Not started'}</div></div><div><div class="text-xs text-slate-400">Lateness</div><div class="text-xl font-bold mt-1 ${late ? 'text-orange-300' : 'text-emerald-300'}">${late} min</div></div></div><button data-action="session-toggle" class="w-full mt-6 rounded-xl ${session ? 'bg-white/10 hover:bg-white/20' : 'bg-teal hover:bg-teal-700'} px-4 py-3 text-sm font-semibold">${session ? 'End session' : 'Start session'} ${icon(session ? 'log-out' : 'play')}</button></div>`;
}
...
function overview() {
  ...
  const branchRows = DESTINATIONS.map(item => {
    const activeCount = activePatients().filter(patient => patient.branch === item.key).length;
    const waiting = waitingFor(item.key).length;
    const people = state.users.filter(employee => employee.dept === item.key && employee.role !== 'Manager');
...
function displays() {
  ...
  if (displayMode === 'door') {
    const doorCards = DESTINATIONS.filter(item => item.key !== 'Reception').map(displayCard).join('');
    return `<div class="fade">${hero('Patient-facing visibility', 'Door screen', 'A clear, live board for every destination. The served count at the top shows today’s completed patients.', modeButtons)}<div class="grid sm:grid-cols-3 gap-4 mb-5">${statCard('Patients served today', servedToday(), 'All destinations', 'badge-check', 'teal')}${statCard('Total served', totalServed(), 'Historical completed journeys', 'users-round', 'blue')}${statCard('Waiting now', activePatients().filter(patient => patient.status === 'Waiting').length, 'Across all doors', 'clock-3', 'orange')}</div><div class="rounded-3xl bg-ink text-white p-6 sm:p-8 mb-5 shadow-soft"><div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4"><div><div class="text-teal-300 text-sm font-semibold flex items-center gap-2">${icon('radio')} LIVE CALL BOARD</div><h2 class="text-2xl font-bold mt-2">Please follow your ticket to the displayed door</h2><p class="text-slate-300 mt-2">Ticket numbers update automatically after an employee presses Next patient.</p></div><div class="text-right"><div class="text-xs text-slate-400">Updated</div><div class="font-semibold">${new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</div></div></div></div><div class="grid xl:grid-cols-2 gap-5">${doorCards}</div></div>`;
  }
  const summary = DESTINATIONS.map(item => `<div class="bg-white rounded-2xl border border-slate-100 shadow-soft p-5"><div class="flex justify-between items-center"><div class="font-semibold">${item.label}</div><span class="text-xs text-slate-400">${activePatients().filter(patient => patient.branch === item.key).length} active</span></div><div class="mt-4 h-2 rounded-full bg-slate-100 overflow-hidden"><div class="h-full bg-teal rounded-full" style="width:${Math.min(100, 15 + waitingFor(item.key).length * 20)}%"></div></div><div class="grid grid-cols-2 gap-3 mt-4 text-sm"><div><div class="text-xs text-slate-400">Serving</div><b>${state.patients.filter(patient => patient.branch === item.key && patient.status === 'Serving').map(patient => `#${patient.code}`).join(', ') || '—'}</b></div><div><div class="text-xs text-slate-400">Waiting</div><b>${waitingFor(item.key).length}</b></div></div></div>`).join('');
...
function reportRows() {
  return employees().map(user => {
    ...
    const minutes = records.reduce((sum, record) => sum + (record.lateMinutes || 0), 0);
...
function addAction(type, patient, extra = {}) {
  state.actions.push({ user: currentUser().name, userId: currentUser().id, dept: currentUser().dept, type, code: patient.code, time: now(), to: extra.to || patient.branch });
...
function sessionToggle() {
  ...
  if (active) {
    ...
    const start = now();
    const lateMinutes = Math.max(0, (new Date(start).getHours() * 60 + new Date(start).getMinutes()) - minutesFromTime(shift));
