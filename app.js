...
const MANAGER_ASSIGNABLE_ROLES = ['Employee', 'Doctor', 'Receptionist'];
...
const VISIBILITY_MODULES = [
  { key: 'queues', label: 'Queues & patient routing' },
  { key: 'displays', label: 'Live display boards' },
  { key: 'sessions', label: 'My session' },
  { key: 'reports', label: 'Reports & attendance' },
...
function recordOrganization(record) {
  ...
  return record?.org || actor?.org || patient?.org || ORG_DEFAULT;
}
...
const managerFilterDepartments = (user = currentUser()) => {
  ...
  const orgs = selectedOrg ? managerOrganizations(user).filter(org => org.name === selectedOrg) : managerFilterOrganizations(user);
  const keys = new Set(orgs.flatMap(org => [...(org.departments || ORGANIZATION_DEPARTMENTS), ...(org.services || []).map(service => service.key)]));
...
const doctorsForOrganization = orgName => state.users.filter(user => user.role === 'Doctor' && (user.org || ORG_DEFAULT) === orgName);
const roomById = id => (state.rooms || []).find(room => room.id === id);
...
function roomStatus(room) {
  return room.active ? '<span class="inline-flex items-center gap-1.5 rounded-full bg-emerald-50 text-emerald-700 px-2.5 py-1 text-xs font-semibold"><span class="h-1.5 w-1.5 rounded-full bg-current"></span>Active</span>' : '<span class="inline-flex items-center gap-1.5 rounded-full bg-slate-100 text-slate-500 px-2.5 py-1 text-xs font-semibold"><span class="h-1.5 w-1.5 rounded-full bg-current"></span>Inactive</span>';
}
...
function stationConsole() {
  ...
  if (!canProcess) return `<div class="command-center-notice notice-card border border-slate-200 bg-white rounded-3xl p-6 sm:p-8 mb-6"><div class="flex items-center gap-4"><div class="h-14 w-14 rounded-2xl bg-teal/10 text-teal flex items-center justify-center shrink-0">${icon('shield-check', 'h-7 w-7')}</div><div><b class="text-lg">Manager control center</b><p class="text-base text-slate-500 mt-2">Use the admin dashboard and reports to supervise every destination and employee.</p></div></div></div>`;
  return `<section class="command-center bg-white rounded-3xl border border-teal-100 shadow-soft p-6 sm:p-8 lg:p-10 mb-6"><div class="flex flex-col xl:flex-row xl:items-center justify-between gap-6"><div><div class="flex items-center gap-2 text-teal text-base font-semibold">${icon('scan-line')} ${user.dept === 'CMO' ? 'Clinic shared queue command center' : `${esc(displayName(user.dept))} command center`}</div><h2 class="text-2xl sm:text-3xl font-bold mt-3">${serving ? `Now serving ticket #${serving.code}` : 'Ready for the next patient'}</h2><p class="text-base text-slate-500 mt-2 max-w-4xl leading-relaxed">Press <b>Next patient</b> to take the oldest eligible ticket. In Clinic, all three available employees draw from one shared organization queue.</p></div><div class="flex items-center gap-2">${availabilityPill(user)}<span class="rounded-xl bg-mist border border-slate-100 px-5 py-3 text-base font-semibold text-slate-600">${waiting} ${user.dept === 'CMO' ? 'in shared Clinic queue' : 'waiting for you'}</span></div></div>${serving ? `<div class="mt-7 rounded-2xl bg-teal/[.06] border border-teal/10 p-5 sm:p-6 flex flex-col md:flex-row md:items-center justify-between gap-4"><div class="flex items-center gap-4"><div class="ticket-number ticket-number-sm">${serving.code}</div><div><div class="font-bold">Current patient</div><div class="text-sm text-slate-500">Issued ${fmt(serving.created)} · ${serving.late ? 'Marked late' : 'On time'}</div></div></div><div class="flex flex-wrap gap-2">${btn('Done', `patient-done-${serving.code}`, 'command-action bg-emerald-600 text-white hover:bg-emerald-700 px-5 py-3.5', 'check')}${btn('Late', `patient-late-${serving.code}`, 'command-action bg-yellow-400 text-ink hover:bg-yellow-500 px-5 py-3.5', 'clock-3')}${btn('Transfer', `patient-transfer-${serving.code}`, 'command-action bg-white text-ink border border-slate-200 hover:border-teal px-5 py-3.5', 'arrow-right-left')}${btn('Pause queue', 'availability-toggle', 'command-action bg-white text-slate-700 border border-slate-200 hover:border-orange-300 px-5 py-3.5', 'pause')}</div></div>` : `<div class="mt-7 flex flex-col sm:flex-row gap-3"><button data-action="next-patient" class="command-action rounded-xl bg-teal text-white px-6 py-4 text-base font-bold hover:bg-teal-700 transition">Next patient ${icon('arrow-right')}</button>${btn('Pause queue', 'availability-toggle', 'bg-white text-slate-700 border border-slate-200 hover:border-orange-300', 'pause')}<div class="text-sm text-slate-500 flex items-center gap-2 px-2">${icon('info')} ${user.availability === 'unavailable' ? 'Set yourself available to receive patients.' : 'No patient is currently assigned.'}</div></div>`}</section>`;
}
function overview() {
  const user = currentUser();
...
function roomAssignmentsPanel() {
  const user = currentUser();
  const orgs = managerOrganizations(user).filter(org => managerFilters.organization === 'all' || org.name === managerFilters.organization);
...
function quickSystemPanel() {
  const user = currentUser();
  const orgOptions = user.globalAdmin ? managerOrganizations(user).map(org => `<option value="${esc(org.name)}">${esc(org.name)}</option>`).join('') : `<option value="${esc(user.org || ORG_DEFAULT)}">${esc(user.org || ORG_DEFAULT)}</option>`;
...
function render() {
  ...
  document.getElementById('main').innerHTML = doctorNotifications() + (views[page] || overview)();
  nav();
  bindPage();
  lucide.createIcons();
}
...
function createOrganization(formData) {
  ...
  const facilityVisibility = formData.getAll('facilityVisibility');
  const doctorVisibility = formData.getAll('doctorVisibility');
  ...
  const organization = { id: `ORG-${String(organizations().length + 1).padStart(3, '0')}`, name, type: fields.type || 'Health unit', location: String(fields.location || '').trim(), departments, visibility: { facility: facilityVisibility, doctor: doctorVisibility }, createdAt: now() };
...
function createAdminUser(formData) {
  ...
  const allowedRoles = manager?.globalAdmin ? ALL_ROLES.filter(role => role !== 'Manager') : MANAGER_ASSIGNABLE_ROLES;
  if (!allowedRoles.includes(user.role)) return toast('Choose a valid predefined employee role.', 'error');
  if (user.role === 'Admin' && !manager?.globalAdmin) return toast('Only the full-access Admin can create another Admin account.', 'error');
...
function createQuickSystem(formData) {
  const fields = Object.fromEntries(formData.entries());
  const manager = currentUser();
...
function updateRoomAssignment(roomId, changes) {
  ...
  toast(`${room.name} is ${room.active ? 'active' : 'inactive'}${doctor ? ` · ${doctor.name} assigned` : ''}.`);
}
